...
public class CodeFlowController implements Initializable {
  ...
  void generarReporte(ActionEvent event) {
    try {
      ...
      // Solicita al usuario un valor numérico para el parámetro idProyecto
      TextInputDialog dialog = new TextInputDialog();
      dialog.setTitle("Introduce un valor");
      dialog.setHeaderText("Por favor, introduce un valor numérico para el parámetro idProyecto:");
      Optional<String> result = dialog.showAndWait();
      String idProyecto = "%";
      if (result.isPresent()) {
        idProyecto = result.get();
        if (idProyecto.equals("")) {
          idProyecto = "%";
        }
      }

      // Carga la plantilla de reporte desde la carpeta resources
      InputStream inputStream = getClass().getResourceAsStream("/CodeFlow.jrxml");
      JasperDesign jasperDesign = JRXmlLoader.load(inputStream);
      JasperReport reporte = JasperCompileManager.compileReport(jasperDesign);

      // Crea un mapa de parámetros. Agrega el parámetro idProyecto con valor "%".
      Map<String, Object> parametros = new HashMap<String, Object>();
      parametros.put("idProyecto", idProyecto);

      // Crea una conexión a la base de datos
      Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/codeflow", "root", "123456");

      // Llena el reporte
      JasperPrint print = JasperFillManager.fillReport(reporte, parametros, connection);

      // Muestra el reporte con JasperViewer
  ...
  void closePanel(ActionEvent event) {
    ...
    if (p.equals(newProjectTab)) {
      ...
      if (!nombreNuevaTarea.getText().equals("") || !descripcionNuevaTarea.getText().equals("") || fecha_inicioNuevaTarea.getValue() != null
          || fecha_finalNuevaTarea.getValue() != null || !prioridad.getValue().toString().equals("MEDIA")) {
  ...
  void createProject(ActionEvent event) {
    ...
    if (nombre.getText().equals("")) {
      ...
      Proyecto p = new Proyecto(nombre.getText(), descripcion.getText(), file.imagenProyecto, Date.valueOf(fecha_inicio.getValue()),
          Date.valueOf(fecha_final.getValue()));
      if (p.crearProyecto()) {
        newProjectTab.setVisible(false);
        newProjectTab.setDisable(true);
  ...
  void mostrarTareas(Proyecto p) {
    ...
    for (Tarea t : p.getTareas()) {
      ...
      } else if ((t.getFecha_inicio().before(Date.valueOf(LocalDate.now())) || t.getFecha_inicio().equals(Date.valueOf(LocalDate.now())))
          && (t.getFecha_fin().after(Date.valueOf(LocalDate.now())) || t.getFecha_fin().equals(Date.valueOf(LocalDate.now())))) {
  ...
  void createTask(ActionEvent event) {
    ...
    if (nombreNuevaTarea.getText().equals("")) {
      ...
      Tarea t = new Tarea(proyecto, nombreNuevaTarea.getText(), descripcionNuevaTarea.getText(), Date.valueOf(fecha_inicioNuevaTarea.getValue()),
          Date.valueOf(fecha_finalNuevaTarea.getValue()), prioridad.getValue());
      if (t.crearTarea()) {
        newTaskTab.setVisible(false);
        newTaskTab.setDisable(true);
